#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════
# JustBuild — One-Command Mac Setup
# Run: bash <(curl -fsSL https://raw.githubusercontent.com/sh1dman/justbuild/main/scripts/setup-mac.sh)
# Idempotent — safe to run multiple times.
# ═══════════════════════════════════════════════════════════
set -e

REPO_URL="https://github.com/sh1dman/justbuild.git"
INSTALL_DIR="$HOME/Desktop/just-build"

echo ""
echo "  ╔══════════════════════════════════╗"
echo "  ║         JustBuild Setup 💀       ║"
echo "  ╚══════════════════════════════════╝"
echo ""
...
if ! command -v brew &>/dev/null; then
  ...
  echo "  This also installs Xcode Command Line Tools — can take 5-10 min."
  sudo -v
...
# --- 7. Install antigravity-awesome-skills ---
...
while ! curl -s -o /dev/null http://localhost:3000 2>/dev/null; do
  ...
  if [[ $TRIES -ge $MAX_TRIES ]]; then
    echo "⚠ Dev server took too long. Open the just-build folder on your Desktop and double-click run.command"
    break
  fi
done
...
echo "  ✓ Setup complete!"
echo ""
echo "  Your app is running in the browser."
echo "  When Codex opens below, sign in with your ChatGPT account."
echo ""

# --- 11. Launch Codex (full-auto) ---
codex --full-auto
